...
RSpec.describe 'Admin::Users', type: :request do
  ...
  describe 'POST /admin/users/import' do
    context 'Caminho Feliz - admin autenticado' do
      ...
      it 'registra erro quando a criação do usuário falha na validação' do
        User.create!(
        ...
        body = JSON.parse(response.body)
        expect(body['erros']).not_to be_empty
        expect(body['erros'].first['email']).to eq('mholanda@unb.br')
      end
    end
