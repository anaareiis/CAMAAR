...
RSpec.describe 'Users::Sessions', type: :request do
  ...
  describe 'POST /users/sign_in' do
    ...
    context 'Caminho Triste - credenciais inválidas' do
      ...
      it 'trata Warden::NotAuthenticated levantado durante a autenticação' do
        allow_any_instance_of(Warden::Proxy).to receive(:authenticate!).and_raise(Warden::NotAuthenticated)
      ...
      it 'trata erro inesperado durante a autenticação' do
        allow_any_instance_of(Warden::Proxy).to receive(:authenticate!).and_raise(StandardError, 'falha de conexão')
        ...
        expect(response).to have_http_status(:unprocessable_entity)
        expect(response.body).to include('Erro ao fazer login.')
      end
    end
  end
